...
import {MetaPixel,trackMetaEvent} from './meta-pixel';
...
export default function Home(){
 ...
 function choose(product:any){
  ...
  trackMetaEvent('ViewContent',{content_ids:[String(product.id)],content_name:product.name,content_type:'product',value:Number(product.price),currency:'DZD'});
 }

 function startCheckout(){
  if(!selected||checkoutTracked.current)return;
  checkoutTracked.current=true;
  trackMetaEvent('InitiateCheckout',{content_ids:[String(selected.id)],content_name:selected.name,content_type:'product',value:Number(selected.price),currency:'DZD',num_items:Number(form.quantity)||1});
 }
 ...
 async function submit(e:React.FormEvent){
  ...
  setBusy(true);
  try{
   const finalTotal=Number(selected.price)*Number(form.quantity)+deliveryFee();
   ...
   const eventId=`order-${order.id||order.order_number||crypto.randomUUID()}`;
   trackMetaEvent('Purchase',{content_ids:[String(selected.id)],content_name:selected.name,content_type:'product',value:finalTotal,currency:'DZD',num_items:Number(form.quantity)},eventId);
   setDone(true);
  }catch{
