...
def product_out(p):
 ...
 return {"id":p.id,"name":p.name,"description":p.description,"price":float(p.price),"old_price":float(p.old_price) if p.old_price is not None else None,"delivery":float(p.delivery),"image":images[0] if images else p.image,"images":images,"colors":json_list(p.colors_json),"sizes":json_list(p.sizes_json),"active":p.active}
...
def product_data(p):
 ...
 data["image"]=images[0] if images else p.image;data["images_json"]=json.dumps(images,ensure_ascii=False);data["colors_json"]=json.dumps([x.strip() for x in p.colors if x.strip()],ensure_ascii=False);data["sizes_json"]=json.dumps([x.strip() for x in p.sizes if x.strip()],ensure_ascii=False)
 return data
@app.get("/api/wilayas")
...
def create_order(o:OrderIn):
 with Session(engine) as s:
  ...
  colors=json_list(p.colors_json);sizes=json_list(p.sizes_json)
